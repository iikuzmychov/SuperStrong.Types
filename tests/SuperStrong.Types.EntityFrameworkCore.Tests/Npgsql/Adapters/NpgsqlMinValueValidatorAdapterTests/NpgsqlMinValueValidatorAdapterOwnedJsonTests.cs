...
public sealed partial class NpgsqlMinValueValidatorAdapterOwnedJsonTests(PostgresDatabaseFixture database)
    : NpgsqlValidationAdapterTest<NpgsqlMinValueValidatorAdapterOwnedJsonTests.TestDbContext>(database)
{
    [StrongType<int>]
    ...
    public sealed class TestDbContext(DbContextOptions<TestDbContext> options) : DbContext(options)
    {
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder
                .Entity<Person>()
                .OwnsOne(person => person.Profile, profile => profile.ToJson());
        }
    }
